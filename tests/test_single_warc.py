...
test_warc_url = "https://data.commoncrawl.org/crawl-data/CC-MAIN-2022-40/segments/1664030331677.90/warc/CC-MAIN-20220924151538-20220924181538-00000.warc.gz"
...
def test_single_warc_iframe():
    results = retry(lambda: process_wat(test_warc_url, "iframe"))
    df = pd.DataFrame(results, columns=cols)
    assert len(df) == 16407
